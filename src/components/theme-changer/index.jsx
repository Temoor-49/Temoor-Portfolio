      ...
      <div className="flex-row items-center space-x-4 flex pl-6 pr-2 py-4">
        <div className="flex-1">
          <h5 className="card-title">
            {loading ? skeleton({ widthCls: 'w-20', heightCls: 'h-8', className: 'mb-1' }) : <span className="text-base-content opacity-70">Theme</span>}
          </h5>
        ...
        <div className="flex-0">
          {loading ? skeleton({ widthCls: 'w-12', heightCls: 'h-10', className: 'mr-6' }) : (
            <div title="Change Theme" className="dropdown dropdown-end">
              ...
              <div tabIndex={0} className="mt-16 overflow-y-auto shadow-2xl top-px dropdown-content max-h-96 min-w-max rounded-lg bg-base-200 text-base-content z-10">
                <ul className="p-4 menu menu-sm">
                  {[themeConfig.defaultTheme, ...themeConfig.themes.filter((item) => item !== themeConfig.defaultTheme)].map((item, index) => (
                    <li key={index}>
